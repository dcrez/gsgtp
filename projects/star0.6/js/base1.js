...
var apiurl =
  "https://starcollaborativeportal.secure.force.com/services/apexrest/JobPortal";
...
var arr_jobtypes = [].concat(
  _toConsumableArray(
    new Set(
      starjobs.map(function(item) {
        return item.Job_Type__c;
      })
    )
  )
);
...
var arr_locations = [].concat(
  _toConsumableArray(
    new Set(
      starjobs.map(function(item) {
        return item.AVTRRT__State__c;
      })
    )
  )
);
...
var arr_focus_areas = [].concat(
  _toConsumableArray(
    new Set(
      starjobs.map(function(item) {
        return item.MC_IntersetGroup__c;
      })
    )
  )
);

var clean_focus = arr_focus_areas.filter(function(n) {
  return n !== undefined;
});
...
var clean_fcs_areas = split_focus.filter(function(n) {
  return n !== undefined;
});
var ordered_fcs_areas = _.sortBy(clean_fcs_areas);
var arr_fcs_clean = [].concat(_toConsumableArray(new Set(ordered_fcs_areas)));
